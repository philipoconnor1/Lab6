				    ...
				    public void handle(MouseEvent event) {
				    	...
				    	if(event.getButton().equals(event.getButton().PRIMARY)){
				            if(event.getClickCount() == 2){
				            	if(!s.isValidValue(CellTo.getiRow(), CellTo.getiCol(), CellTo.getiCellValue())) {
				            		game.getSudoku().RemoveMistake();
				            	}
				            	paneTarget.getChildren().remove(0);
